...
| `design/reasoning.md` | Every place a model exercises judgment (four agents, seven calls), the three Reason/Act/Observe/Decide loops — two of which contain no model — and where a model was deliberately declined | Describing the system as agentic, writing the report or presentation, or before adding a model call anywhere |
...
**Section numbers (§1–§9) and decision numbers (#1–#22) always refer to this file** —
...
| 22 | Asking price against the market benchmark (check B) | Rent & valuation | U7 → **U9** | ✅ **Disclosure, never a check — measured Sept 2, 2026, three days after the gap was noticed.** The sibling of #20, which U8.7 settled while the phrase *"checks A and B"* carried B along with **no measurement and no decision of its own**. `scripts/asking_price_gap.py` supplied one over 22 fixtures. The prediction — that a threshold would restate #11's calibration — held, and the reason turned out to be structural: **the fixtures do not share a calibration basis, so no column exists to threshold**. #11 set the original demo prices from the *metro* median while U9.4/U9.6 calibrated the newest two against the *ZIP* benchmark, so the raw gap recovers `overpriced`'s declared **+55%** exactly and simultaneously reports an ordinary Uptown duplex as **39% cheap**. Two further findings: at the metro tier the gap ranks by *where* a property is (`ny-manhattan-dispersed` **+146%**), and on three fixtures the check is **inert by construction** — price set from the metro median, scored against the metro median, gap structurally +0% — in the markets with no ZIP tier, which is half the inference set. **The benchmark's tier flag closes with it**, by its stated expiry (*"the moment check B is promoted"*) not being met |
