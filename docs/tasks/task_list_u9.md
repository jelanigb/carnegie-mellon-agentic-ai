...
### U9.7T ✅ — The scenario table: name the rows for what they say, and the ledger for what decided them *(done Sept 2, 2026)*
...
**Correction, Sept 2, 2026 (U9.11), because this instruction was wrong and would have cost a
confusing artifact mid-capture.** This step used to say *"`> report.md` captures the report
alone, since the four status lines and the two stderr lines stay out of a redirect."* They do
not. `main.py` prints all four status lines — and, on an escalating deal, the whole
`human_review` interrupt payload — to **stdout**, so a redirect captures them too. Only the
two HuggingFace advisories are on stderr. To capture the report alone:

```bash
LLM_CACHE_MODE=replay LLM_CACHE_DIR=eval/data/llm_recordings \
  .venv/bin/python main.py --deal los-angeles | awk '/^# Deal Evaluation/{f=1} f' > report.md
```

That is how the three files in `docs/sample_reports/` are produced. **For the video the
status lines are an asset, not noise** — `tracing: on -> LangSmith project 'deal-evaluator'`
and `Model check OK` are the two lines that prove the run is real, and the interrupt payload
on `staten-island` is the human-in-the-loop evidence rendered as JSON. Keep them on screen;
strip them only for a committed file.
...
#### The deliverables, restated Sept 2, 2026 — this subsection is three things, not one

**The plan above covers only the first.** Written when U9.9 was "capture traces and
screenshots"; Checkpoint 7.1 asks for two graded artifacts *built on top of* that capture,
and neither had a plan.

| | Deliverable | State |
| --- | --- | --- |
| **1** | **The capture** — traces, screenshots, the diagram, terminal recordings | Runbook above. Needs a live account and a screen |
| **2** | **The 8–10 minute presentation video** | Slide outline, timing budget and a full script draft below |
| **3** | **The 1,000–1,500 word final report** | Section-by-section source map below. Every section already has its evidence in this repository |
...
three deliverables must be submitted to be considered. Drafted at the end.

#### Two reference points, read Sept 2 — and the one thing both of them left open

Two completed capstones from the cohort (`docs/private/capstone_reference/`). Both are good,
...
`docs/private/capstone_reference/templates/`, with the demo inserted at position 6.
...
**Run the surface in replay** (`.venv/bin/streamlit run app.py`), which is its default. Every
...
### U9.M ✅ — Maintenance *(separate commits, per §8; done Sept 2, 2026)*
