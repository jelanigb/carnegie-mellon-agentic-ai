...
def main() -> None:
    ...
    # **The live-tier exclusion is load-bearing as of U9.6 and used to be redundant.**
    # `scoring_cases()` already drops `BASELINE` verdicts, and until U9.6 every live case
    # was a demo deal carrying one — so this filter removed nothing and read as belt and
    # braces. U9.6 added two demo deals *after* the U7.8 table, which therefore declare
    # `PREDICTED` verdicts (they have no published outcome to transcribe), and this line
    # is now the only thing keeping them out of the sweep.
    #
    # **Keeping them out is the original intent, not an accident of the proxy.** This
    # script's own docstring disqualifies the demo deals because they are calibrated to
    # run clean; a parameter swept over cases built to report tells you where the
    # threshold can move without disturbing deals that were never near it. The proxy
    # (`tier is LIVE` == "is a demo deal") still holds exactly in this build, and it is
    # the reason the count here is 21 where `results.md` reports agreement over 23.
    ...
    lines = [
        "# Confidence sensitivity — decision #6 (confidence threshold) against the eval batch (U8.6)",
        "",
        f"Generated by `scripts/confidence_sensitivity.py` over {len(results)} "
        f"engineered cases with a verdict declared before the run "
        f"(`{VerdictSource.PREDICTED.value}`), of which {escalating} escalate under the "
        "shipped configuration.",
        "",
        "**Fewer cases than `results.md` scores, deliberately.** That table reports "
        "agreement over every `predicted` case; this sweep drops the live-tier ones — the "
        "demo deals — because they are calibrated to run clean, and a threshold swept "
        "over deals that were never near it measures nothing. The two counts differ by "
        "the demo deals added after the U7.8 table, which declare predictions because "
        "they have no published outcome to transcribe (U9.6).",
        "",
